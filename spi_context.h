...
// drivers use to store actual mode...
...
struct spi_context {
  // active configuration of current transfer
	const struct spi_config *config;

  // prevents dif callers from simultaneously using the same SPI bus unless
  // SPI_LOCK_ON is used ...?
  // fact-check this
  ...
  // IF THE SPI CONTROLLER USES GPIO CS PINS
  // QUESTION FOR SELF: what are the alternatives?
#ifndef DT_SPI_CTX_HAS_NO_CS_GPIOS
	const struct gpio_dt_spec *cs_gpios;
	size_t num_cs_gpios;
#endif /* !DT_SPI_CTX_HAS_NO_CS_GPIOS */

  // lock prevents transfer sequences
  // sync signasl transfer completion
#ifdef CONFIG_MULTITHREADING
	struct k_sem lock;
	struct k_sem sync;
#else
  ...
  // in a spi_buf_set, how many bytes remain in transmission?
  ...
  // I NEED THIS!! and maybe 2 add to this
  // look into: does slave ened all of above, too
#ifdef CONFIG_SPI_SLAVE
	int recv_frames;
#endif /* CONFIG_SPI_SLAVE */
};

// SPI CONTEXT LOCKS AND SYNC--ensures only one thread owns the bus.
...
// I THINK this might populate something from devcetree?
// see if slave needs this
// ACTUALLY Nit only spopulates stuff from devecetree if ELSE. look into this
// is very much needed lol
#ifndef DT_SPI_CTX_HAS_NO_CS_GPIOS
