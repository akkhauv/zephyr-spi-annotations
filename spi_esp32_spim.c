...
// DEVICES SPI API!!
// every driver has: config struct, data struct, API struct
// defines API struct.....
// note to self-- device_api expands to const struct spi_driver_api spi_api
// IMPORTANT -- THISIS WHERE THE API IS LINKED!!!
// these are really THE functions i need to implmenet
// NOTE -- IMPORTNAT -- when user callse spi_transceive(), syscall dispatch
// layer does z_impl_spi_tarnsceive(), and api is loaded into
// dev->api->transceive....
// ALSO A PRETTY COOL NOTE-- SEE HOW CONFIGS DEFINE WHAT'S EVEN BUILT INTO
// THIS!!!!! THIS IS THE KCONFIG STUFF THATS REALLY IMPORTANT.
// so many macros
...
#define ESP32_SPI_INIT(idx)	\
				\
  // PINCTRL DEFINITOIN: expands to the pinctrl config data for the
  // DT instance
  // compile time
  ...
  // declares data stuff
  // driver runtime state -- INCLUDEDS SPI_CONTEXT, HAL context, etc
  // ultimately covers -- a struct that is initialzed alr for runtime, so i can
  // customize it in runtime to trasnfer/talk2 hardware
	static struct spi_esp32_data spi_data_##idx = {	\
    // initialize semaphore
		SPI_CONTEXT_INIT_LOCK(spi_data_##idx, ctx),	\
    
    // for signal completion or somethinging
		SPI_CONTEXT_INIT_SYNC(spi_data_##idx, ctx),	\

    // INITIALIZE CS GPIOS!!! from DT defs
    // if no CS GIPS, respolves to nothing
    // DT_DRV_INST: alias for DT node for the driver instance
		SPI_CONTEXT_CS_GPIOS_INITIALIZE(DT_DRV_INST(idx), ctx)	\

    // initializes HAL context
    // this may or may not be the reason that i dont think (check this) I need
    // to reference the hal directly later...??
    // TODO: CHECK LATER!!! TO SEE IF I AM RIGHT about this
    // hypothesis
    // id like to look more into the internals, though
		.hal = {	\
			.hw = (spi_dev_t *)DT_INST_REG_ADDR(idx),	\
		},	\

    //initializes hardware config substr
		.dev_config = {	\
  ...
  // configuration struct for... hm
  // reading DT config and driver uses at runtime...?
  // mostly reads from devicetree
	static const struct spi_esp32_config spi_config_##idx = {	\
    // hardware base addr
		.spi = (spi_dev_t *)DT_INST_REG_ADDR(idx),	\
			\

    //phandle to clock controller used by SPI instance...
    // driver can call device_i_read on this() TODO: check
    // phandle to the clock controller used by SPI instance TODO: CHECK WHTA
    // THIS MEANS; does each dev get its own clock controller? what is clock
    // controller?
		.clock_dev = DEVICE_DT_GET(DT_INST_CLOCKS_CTLR(idx)),	\
		.duty_cycle = 0, \
		.input_delay_ns = 0, \

    // fetches IRQ NUMBER (interrupt request number) from DT
    ...
    // pinctrl device config create uhh stuff 
    ...
    // expand sdifferent depending on soc
    // wait THIS IS SO IMPORTANT
    // COME BACK TO THIS!!!!!!!!!!!!
    // BIG TODO
    // ESP32
    // populates all DMA RELATED FIELDS 
		SPI_DMA_CFG(idx),				\
    // 0 if not present
  ...
  // REGSTERS zephyr struct device instance for the peripheral
  // spi_esp32_init: dev initializtiation func called at boot
	SPI_DEVICE_DT_INST_DEFINE(idx, spi_esp32_init,	\
            // NULL: power management ptr
            // spi_data_idx: urntime data structure for device data
			      NULL, &spi_data_##idx,	\
            // points to device config struct
            // post_kernel: init level; runs after kernel subsystems r up
			      &spi_config_##idx, POST_KERNEL,	\
            // init priority within post pkernel levels
            // SPI_API: THE DEVICES API STRUCTURE
            // THIS IS IMPORTNAT
            // THISIS WHERE IT IS INITIALIZED!!!!!!!!!!!!!!!!!
            // WE FOUNT ITTT
            // assgined to dev->api
